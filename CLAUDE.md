...
|---------|---------------------|----------------|-----|
| **WhatsApp (Twilio)** | `TWILIO_WHATSAPP_NUMBER` | `lib/whatsapp/send.ts`, `lib/whatsapp/templates.ts`, `app/api/webhooks/whatsapp/route.ts` | Twilio konsolunda WA Sandbox'ı etkinleştir, sandbox için `whatsapp:+14155238886`. Varsayılan sağlayıcı (`WHATSAPP_PROVIDER=twilio`). |
| **WhatsApp (Meta Cloud)** | `WHATSAPP_PROVIDER=meta`, `META_WA_PHONE_NUMBER_ID`, `META_WA_ACCESS_TOKEN`, `META_WA_VERIFY_TOKEN`, `META_WA_APP_SECRET` | `lib/whatsapp/meta-cloud.ts`, `lib/webhooks/verify-meta.ts`, `app/api/webhooks/whatsapp-meta/route.ts`, `lib/whatsapp/send.ts` | Meta Business Manager → WhatsApp → API Setup. Webhook URL: `/api/webhooks/whatsapp-meta`. Twilio komisyonu ortadan kalkar — Meta konuşma başı fiyatlandırma. Text + onaylı template destekli. |
...
- [x] `/api/invoices/payments` POST — cross-tenant kontrolü + `staff_id`/`staff_name` artık auth context'ten türetiliyor (audit log bütünlüğü), fazla ödeme/sıfır total guard'ları, `ipAddress` audit log'a eklendi (2026-04-25)
- [x] `/api/appointments/[id]` PATCH — drag-drop için atomic RPC (`move_appointment` — advisory lock + FOR UPDATE); permission kontrolü eklendi (2026-04-25)
- [x] `/api/protocols/[id]` — `requirePermission`/`requireWritePermission` pattern'ine geçirildi (2026-04-25)
- [x] `/api/records` GET — `customerId` UUID validation (2026-04-25)
